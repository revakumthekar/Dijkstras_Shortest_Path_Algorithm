...
/**
 * This class creates a graph off of a set of vertexes and edges and contains
 * methods that interacts with the graph and gets information from it. 
 * @author revakumthekar
 *
 * @param <T> is the generic type for the type of graph created
 */
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
      //throws an exception if there is no start or end vertex
      if(vertices.get(start) == null || vertices.get(end) == null)
      {
        throw new NoSuchElementException("no vertex containing start can be found");
      }
      
      Path startPath = new Path(vertices.get(start));
      PriorityQueue<Path> priorQ = new PriorityQueue<Path>();
      LinkedList<Vertex> visited = new LinkedList<Vertex>();
      priorQ.add(startPath);
      //while loop to go through each of the paths in the priority queue to make sure the shortest path is found
      while(!priorQ.isEmpty())
      {
        Path shortestTemp = priorQ.remove();
        visited.add(shortestTemp.start);
        //a for loop to go through all of the neighboring edges attached to the vertex, finding the next shortest path
        for(int i = 0; i < shortestTemp.end.edgesLeaving.size(); i++)
        {
          if(!visited.contains(shortestTemp.end.edgesLeaving.get(i).target))
          {
            Path tempPath = new Path(shortestTemp ,shortestTemp.end.edgesLeaving.get(i));
            priorQ.add(tempPath);
          }
          
        }
        
        //if the end of the temporary shortest path is equal to the end that we are looking for, return that shortest path
        if(shortestTemp.end == vertices.get(end))
        {
          return shortestTemp;
        }
      ...
      //throws an exception if there is no path from start to end 
      throw new NoSuchElementException("no path from start to end can be found"); // TODO: Implement this method in Step 7.
    }
